...
function logger(func, logFunc) {
  return function ololo(...args) {
    const formArgs = args.map((item) => JSON.stringify(item)).join(',');
    logFunc(`${func.name}(${formArgs}) starts`);
    const a = func(...args);
    logFunc(`${func.name}(${formArgs}) ends`);
    return a;
  };
}
